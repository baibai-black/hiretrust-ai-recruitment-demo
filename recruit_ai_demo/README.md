...
如果只上传或下载 `recruit_ai_demo/` 作为独立仓库，请在该目录内运行：
...
1. 创建一个新的 GitHub 仓库。
2. 上传项目文件，至少包括 `recruit_ai_demo/app.py`、`recruit_ai_demo/recruit_ai_mock.py`、`recruit_ai_demo/workflow_state.py`、`README.md` 或 `recruit_ai_demo/README.md`、`requirements.txt`、`DESIGN.md`。
3. 确保不要上传缓存、虚拟环境、压缩包、密钥、`.env` 或任何真实候选人信息。
4. 如果保持当前目录结构上传整个仓库，Streamlit Cloud 的入口应填写 `recruit_ai_demo/app.py`。
